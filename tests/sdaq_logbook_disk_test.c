...
extern int LogBook_file(struct Morfeas_SDAQ_if_stats *stats, const char *mode);
...
int main(void)
{
    /* LogBook_file() computes its own now = time(NULL) internally (it has no
     * injectable clock), so fixtures must use real wall-clock time too --
     * an arbitrary fixed epoch here would make "valid" entries look already
     * expired to the code under test. */
    ...
    CHECK(LogBook_file(&stats, "r") == EXIT_SUCCESS, "empty LogBook file reads as success");
    ...
    CHECK(LogBook_file(&stats, "w") == EXIT_SUCCESS, "writing a single live entry succeeds");
    CHECK(file_size_of(path) == (long)sizeof(struct test_LogBook_disk), "the file contains exactly one new-format record");
    dispose(&stats);

    init_stats(&stats, path);
    CHECK(LogBook_file(&stats, "r") == EXIT_SUCCESS, "reading the round-tripped file succeeds");
    {
    ...
    init_stats(&stats, path);
    cache_upsert_entry(&stats, 3003, 7, (uint64_t)now - 1); /* expired; becomes the list head */
    cache_upsert_entry(&stats, 4004, 8, cache_valid_until_from(now));
    CHECK(LogBook_file(&stats, "w") == EXIT_SUCCESS, "write survives an expired list head without crashing (UAF regression)");
    ...
    init_stats(&stats, path);
    CHECK(LogBook_file(&stats, "r") == -1, "a corrupted new-format checksum is rejected");
    ...
    init_stats(&stats, path);
    CHECK(LogBook_file(&stats, "r") == EXIT_SUCCESS, "a legacy-format file is recognized and read without error");
    ...
    init_stats(&stats, path);
    CHECK(LogBook_file(&stats, "r") == -1, "a file matching neither record size is rejected");
