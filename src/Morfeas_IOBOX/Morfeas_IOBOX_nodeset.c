...
void IOBOX_handler_reg(UA_Server *server_ptr, char *Dev_or_Bus_name)
{
	...
	pthread_mutex_lock(&OPC_UA_NODESET_access);
		...
		Morfeas_opc_ua_add_variable_node(server_ptr, Dev_or_Bus_name, tmp_buff, "Status", UA_TYPES_STRING);
		...
		Morfeas_opc_ua_add_variable_node(server_ptr, Dev_or_Bus_name, tmp_buff, "Status Value", UA_TYPES_INT32);
...
{
	UA_NodeId NodeId;
	char Node_ID_str[60], IOBOX_IPv4_addr[20];	
		...
		case IPC_IOBOX_data:
			pthread_mutex_lock(&OPC_UA_NODESET_access);
				...
				if(UA_Server_readNodeId(server, UA_NODEID_STRING(1, Node_ID_str), &NodeId))
				{
					...
					inet_ntop(AF_INET, &(IPC_msg_dec->IOBOX_data.IOBOX_IPv4), IOBOX_IPv4_addr, sizeof(IOBOX_IPv4_addr));
					sprintf(Node_ID_str, "%s.IP_addr", IPC_msg_dec->IOBOX_data.Dev_or_Bus_name);
					Update_NodeValue_by_nodeID(server, UA_NODEID_STRING(1,Node_ID_str), IOBOX_IPv4_addr, UA_TYPES_STRING);
